...
/*//* Draw rain diagram
function rainDiagram(){
    var i, highestRainVal = 0, poleHeight, poleWidth, textMargin = 10;
    ctx.clearRect(0, 0, 1000, 1000);

    //* Read highest rain-value
    for (i = 0; i < stationArr.length; i++) {
        if(highestRainVal < stationArr[i].rain){
            highestRainVal = stationArr[i].rain;}
    }

    for (i = 0; i < stationArr.length; i++) {
        //| float between 0 and 1 
        floatVal = stationArr[i].rain / highestRainVal;    
        poleHeight = floatVal * canvasGfx.offsetHeight - 30; //|30px down from top, space for title
        poleWidth = canvasGfx.offsetWidth/stationArr.length;

        //console.log(highestRainVal + ", " + floatVal + ", " + poleHeight);
        //| Draw poles
        ctx.fillStyle = "#00f5";
        ctx.fillRect( //| poles fill
            i * poleWidth, 
...
preOut.innerHTML = "Text \t mer text \n test";